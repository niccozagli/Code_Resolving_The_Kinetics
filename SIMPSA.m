...
DEFAULT_OPTIONS = SIMPSASET('TEMP_START',[],...  % starting temperature (if none provided, an optimal one will be estimated)
             ...
             'MAX_ITER_TEMP_FIRST',100,...        % number of iterations in the preliminary temperature loop
             'MAX_ITER_TEMP_LAST',100,...         % number of iterations in the last temperature loop (pure simplex)
             'MAX_ITER_TEMP',20,...              % number of iterations in the remaining temperature loops
             'MAX_ITER_TOTAL',10000,...           % maximum number of iterations tout court
             'MAX_TIME',10000,...                 % maximum duration of optimization
             'MAX_FUN_EVALS',10000,...            % maximum number of function evaluations
             'TOLX',1e-24,...                     % maximum difference between best and worst function evaluation in simplex
             'TOLFUN',1e-12,...                   % maximum difference between the coordinates of the vertices
...
%YBEST = CALCULATE_COST(FUN,PBEST,LB,UB,P1,P2);
...
while 1,
    ...
    %Y(1) = CALCULATE_COST(FUN,P(1,:),LB,UB,P1,P2);
    ...
    for k = 1:NDIM,
        ...
        %Y(k+1) = CALCULATE_COST(FUN,P(k+1,:),LB,UB,P1,P2);
    end
    ...
    for ITERTEMP = 1:MAXITERTEMP,
        ...
        %[YFTRY,YTRY,PTRY] = AMOTRY(FUN,P,-1,LB,UB,P1,P2);
        ...
        if YFTRY <= YFLUCT(1),
            ...
            %[YFTRYEXP,YTRYEXP,PTRYEXP] = AMOTRY(FUN,P,-2,LB,UB,P1,P2);
            if YFTRYEXP < YFTRY,
                P(end,:) = PTRYEXP;
                Y(end) = YTRYEXP;
        ...
        elseif YFTRY >= YFLUCT(NDIM),
            ...
            %[YFTRYCONTR,YTRYCONTR,PTRYCONTR] = AMOTRY(FUN,P,-0.5,LB,UB,P1,P2);
            if YFTRYCONTR < YFLUCT(end),
                ...
                for k=2:NDIM,
                    ...
                    %Y(k) = CALCULATE_COST(FUN,P(k,:),LB,UB,P1,P2);
                end
...
%function [YFTRY,YTRY,PTRY] = AMOTRY(FUN,P,fac,LB,UB,P1,P2)
...
%YTRY = CALCULATE_COST(FUN,PTRY,LB,UB,P1,P2);
...
%function [YTRY] = CALCULATE_COST(FUN,PTRY,LB,UB,P1,P2)
